  ...
  return (
    <div className="card overflow-hidden p-0">
      <div className="bg-neutral-900 p-4 border-b border-neutral-800 flex items-center gap-2">
        ...
        <h3 className="font-black text-white uppercase text-sm tracking-tight">Progresja Bench (16 Tyg.)</h3>
      </div>
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="bg-neutral-900/50 text-neutral-500 uppercase font-black">
              ...
              <th className="p-3">Top Set (A)</th>
              <th className="p-3">Back-off (A)</th>
              <th className="p-3">Light (D)</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-900">
            {BENCH_PROGRESSION.map((row, idx) => (
              <tr key={idx} className={row.isPR ? 'bg-primary/10' : ''}>
                ...
                <td className="p-3 text-white font-black italic">{row.top}</td>
                <td className="p-3 text-neutral-400">{row.backoff}</td>
